...
// ---- Funcție auxiliară pentru decodarea JWT ----
...
export async function apiLogin({ username, password }) {
  ...
  //  Backendul  trimite doar { token }
  const { token } = await res.json();
  if (!token) throw new Error("No token received from backend");

  //  Decodează tokenul pentru a extrage datele utilizatorului
  ...
  //  Returnează structura standard folosită de AuthContext
  return { token, user };
}
...
export async function apiValidate() {
  ...
  const res = await fetch(`${AUTH_BASE}/api/auth/validate?token=${encodeURIComponent(t)}`);

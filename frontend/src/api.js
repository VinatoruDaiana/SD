...
export async function apiLogin({ username, password }) {
  ...
  // --- NEW: handle non-JSON responses (e.g. 502 from Traefik) ---
  ...
  if (contentType.includes("application/json")) {
    ...
    console.error("🟡 apiLogin: non-JSON response:", text);
    throw new Error(`Login failed (${res.status}): ${text || "Non-JSON response"}`);
  }
  ...
  // build user from JWT, same as before ...
